...
(function () {
  ...
  const DARK_CLASS = 'i56-dark';
  const LIGHT_CLASS = 'i56-light';
  ...
  var I56Theme = {
    ...
    get: function () {
      var html = document.documentElement;
      if (html.classList.contains(LIGHT_CLASS)) return 'light';
      if (html.classList.contains(DARK_CLASS)) return 'dark';
      // No class set — derive from computed style
      if (window.matchMedia && window.matchMedia('(prefers-color-scheme: light)').matches) {
        return 'light';
      }
      return 'dark';
