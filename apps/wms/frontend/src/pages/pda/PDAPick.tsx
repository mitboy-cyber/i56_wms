...
export default function PDAPick() {
  ...
  const mut = useMutation({
    mutationFn: (scan: string) => (pdaApi as any).pick({ scan } as any),
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4">拣货</h2>
      <ScanInput placeholder="拣货扫码..." onSubmit={(s) => (mut as any).mutate(s)} loading={mut.isPending as boolean} />
