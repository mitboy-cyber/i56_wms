...
export default function PDAPutaway() {
  ...
  const mut = useMutation({
    mutationFn: (scan: string) => (pdaApi as any).putaway({ scan } as any),
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4">上架</h2>
      <ScanInput placeholder="上架扫码..." onSubmit={(s) => (mut as any).mutate(s)} loading={mut.isPending as boolean} />
