...
export default function PDAPutaway() {
  ...
  const submit = async () => {
    setMsg('');
    try {
      const res = await fetch('/pda/api/putaway', {
        method: 'POST', credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scan, location_barcode: loc }),
      });
      const d = await res.json();
      setMsg(res.ok ? '上架成功' : (d?.error || '上架失败'));
    } catch { setMsg('网络错误'); }
  };

  return (
    <div>
      <h2 className="text-lg font-bold mb-4" style={{ color: 'var(--color-ink)' }}>🏗️ 上架</h2>
      <div className="bg-white rounded-xl border p-4 space-y-3 shadow-sm" style={{ borderColor: 'var(--border)' }}>
        ...
        <div>
          <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--color-muted)' }}>货位条码</label>
        ...
        <button onClick={submit} className="w-full py-2.5 text-white rounded-lg font-medium"
          style={{ background: 'var(--color-accent)' }}>确认上架</button>
