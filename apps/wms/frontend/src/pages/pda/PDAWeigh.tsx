...
export default function PDAWeigh() {
  ...
  const submit = async () => {
    setMsg('');
    if (!scan || !weight) { setMsg('请填写完整信息'); return; }
    try {
      const res = await fetch('/pda/api/weigh', {
        method: 'POST', credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scan, weight: +weight }),
      });
      const d = await res.json();
      setMsg(res.ok ? (d?.message || '称重成功') : (d?.error || '称重失败'));
    } catch { setMsg('网络错误'); }
  };

  return (
    <div>
      <h2 className="text-lg font-bold mb-4" style={{ color: 'var(--color-ink)' }}>⚖️ 称重复核</h2>
      <div className="bg-white rounded-xl border p-4 space-y-3 shadow-sm" style={{ borderColor: 'var(--border)' }}>
        ...
        <div>
          <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--color-muted)' }}>实际重量(kg)</label>
        ...
        <button onClick={submit} className="w-full py-2.5 text-white rounded-lg font-medium"
          style={{ background: 'var(--color-accent)' }}>确认称重</button>
