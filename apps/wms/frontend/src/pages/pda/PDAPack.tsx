...
export default function PDAPack() {
  ...
  const mut = useMutation({
    mutationFn: (scan: string) => (pdaApi as any).pack({ scan } as any),
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4">打包</h2>
      <ScanInput placeholder="打包扫码..." onSubmit={(s) => (mut as any).mutate(s)} loading={mut.isPending as boolean} />
