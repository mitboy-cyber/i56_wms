...
export default function PDAReceive() {
  ...
  const mut = useMutation({
    mutationFn: (scan: string) => (pdaApi as any).receive({ scan } as any),
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4">收货</h2>
      <ScanInput placeholder="收货扫码..." onSubmit={(s) => (mut as any).mutate(s)} loading={mut.isPending as boolean} />
