...
interface ClientRow { client: string; orders: number; services: number; revenue: number; cost: number; profit: number; margin: number; }

export default function ClientProfitReport() {
  const { data: rows = [] } = useQuery<ClientRow[]>({
    queryKey: ['admin-client-profit-report'],
    queryFn: () => client.get('/admin/api/report/client-profit').then(r => r.data || []),
  });
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-gray-800">客户盈利汇总</h2>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <Kpi icon={Users} label="活跃客户" value={`${rows.length}家`} color="blue" />
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b"><tr>
            <th className="px-4 py-3 text-left">客户</th><th className="px-4 py-3 text-right">集运订单</th><th className="px-4 py-3 text-right">服务单</th>
