...
export default function InboundBoardPage() {
  const { data: parcels = [] } = useQuery<any[]>({ queryKey: ['inbound-parcels'], queryFn: () => client.get('/admin/api/parcels').then(r => r.data) });
  const { data: stats } = useQuery<any>({ queryKey: ['inbound-stats'], queryFn: () => client.get('/admin/api/dashboard/stats').then(r => r.data) });
