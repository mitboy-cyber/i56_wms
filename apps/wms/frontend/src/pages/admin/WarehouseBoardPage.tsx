...
export default function WarehouseBoardPage() {
  const { data: stats } = useQuery<any>({ queryKey: ['wh-board-stats'], queryFn: () => client.get('/admin/api/dashboard/stats').then(r => r.data) });
  const { data: orders = [] } = useQuery<any[]>({ queryKey: ['wh-board-orders'], queryFn: () => client.get('/admin/api/orders').then(r => r.data) });
  const { data: parcels = [] } = useQuery<any[]>({ queryKey: ['wh-board-parcels'], queryFn: () => client.get('/admin/api/parcels').then(r => r.data) });
