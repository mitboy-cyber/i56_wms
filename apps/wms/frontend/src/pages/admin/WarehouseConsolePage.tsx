...
export default function WarehouseConsolePage() {
  const { data: stats } = useQuery<any>({ queryKey: ['wh-console-stats'], queryFn: () => client.get('/admin/api/dashboard/stats').then(r => r.data) });
  const { data: orders = [] } = useQuery<any[]>({ queryKey: ['wh-console-orders'], queryFn: () => client.get('/admin/api/orders').then(r => r.data) });
  const { data: parcels = [] } = useQuery<any[]>({ queryKey: ['wh-console-parcels'], queryFn: () => client.get('/admin/api/parcels').then(r => r.data) });
