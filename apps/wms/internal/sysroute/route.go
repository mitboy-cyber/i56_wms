...
<p style="color:var(--i56-text-muted)">使用 <code>{{.变量名}}</code> 在模板HTML中插入动态数据</p>
	...
	r.GET("/admin/system/ai-chat", a(func(w http.ResponseWriter, req *http.Request) {
		common.HtmlOK(w)
		fmt.Fprint(w, `<!DOCTYPE html><html lang="zh-CN" data-theme="dark"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI 助手 - I56</title><link rel="stylesheet" href="/static/css/i56-bdl.css"><script src="/static/js/i56-theme.js"></script><style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,sans-serif;background:var(--i56-bg-base);color:var(--i56-text-primary);height:100vh;display:flex;flex-direction:column}
.ai-chat-panel{display:flex;flex-direction:column;height:100%;max-width:900px;margin:0 auto;width:100%}
.ai-header{background:var(--i56-bg-surface);border-bottom:1px solid var(--i56-border);padding:16px 24px;display:flex;align-items:center;gap:12px}
.ai-header h2{font-size:18px;font-weight:600;color:var(--i56-brand)}
.ai-status{font-size:11px;padding:3px 8px;border-radius:10px;background:#22c55e20;color:#22c55e}
.ai-messages{flex:1;overflow-y:auto;padding:20px 24px;display:flex;flex-direction:column;gap:12px}
...
.ai-input-wrap{border-top:1px solid var(--i56-border);padding:16px 24px;background:var(--i56-bg-surface)}
...
<div class="ai-header"><span style="font-size:24px">🤖</span><h2>I56 AI 助手</h2><span class="ai-status">● 在线</span><span style="font-size:11px;color:var(--i56-text-muted);margin-left:auto">提问订单、包裹、仓库、财务信息</span></div>
<div class="ai-messages" id="ai-messages"><div class="ai-welcome">🤖 <strong>I56 AI 助手</strong> — 我是您的智能仓库助手<br><span style="font-size:12px;color:var(--i56-text-muted)">您可以询问：订单查询 · 包裹状态 · 仓库库存 · 客户信息 · 财务报表</span></div></div>
<div class="ai-input-wrap">
<div class="ai-quick-btns"><span class="ai-quick-btn" onclick="quickAsk('本月订单量多少？')">📋 订单统计</span><span class="ai-quick-btn" onclick="quickAsk('有哪些包裹？')">📦 包裹列表</span><span class="ai-quick-btn" onclick="quickAsk('列出所有仓库')">🏗️ 仓库清单</span><span class="ai-quick-btn" onclick="quickAsk('客户余额情况')">💰 客户余额</span><span class="ai-quick-btn" onclick="quickAsk('最近有哪些订单？')">🔄 最近订单</span></div>
<form class="ai-input-form" onsubmit="return sendAIMessage(event)"><input type="text" id="ai-input" placeholder="输入问题，如：本月订单量多少？" autocomplete="off"><button type="submit" id="ai-send-btn">发送</button></form></div></div>
<script>
