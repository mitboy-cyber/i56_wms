...
class I56Table extends HTMLElement {
  static get observedAttributes() { return ['data', 'columns', 'striped', 'hoverable', 'sortable', 'selectable', 'empty-message', 'virtual-scroll', 'virtual-row-height']; }
  ...
  constructor() {
    ...
    this._rowHeight = 0;       // measured or configured
    this._visibleRange = { start: 0, end: 0 };
    this._renderRequested = false;
    this.render();
  }

  /** Whether virtual scrolling is active for this instance. */
  get _useVirtual() {
    if (this.hasAttribute('virtual-scroll')) return true;
    // Auto-enable for datasets above 500 rows
  ...
  /** Configured row-height (px) — default 40. */
  get _virtualRowHeight() {
    const v = parseInt(this.getAttribute('virtual-row-height'));
    return v > 0 ? v : 40;
  }
  ...
  attributeChangedCallback(name) {
    ...
    if (name === 'virtual-scroll' || name === 'data') {
      this._rowHeight = 0; // force re-measure
    }
    this.render();
  }
  ...
  /** Measure a single row height from the DOM or fall back to configured value. */
  _measureRowHeight() {
    if (this._rowHeight > 0) return this._rowHeight;
    // Try measuring an existing row
    const existing = this._root.querySelector('.virtual-table tbody tr');
    if (existing) {
      this._rowHeight = existing.getBoundingClientRect().height;
      if (this._rowHeight > 0) return this._rowHeight;
    }
    this._rowHeight = this._virtualRowHeight;
    return this._rowHeight;
  }

  /** Calculate which rows are visible based on current scroll. */
  _calcVisibleRange() {
    const rowH = this._measureRowHeight();
    if (!rowH) return { start: 0, end: 0 };
    const data = this._sortedData();
    const total = data.length;

    // Overscan: render a few rows above / below the viewport so scrolling feels seamless
    const overscan = 10;
    const start = Math.max(0, Math.floor(this._scrollTop / rowH) - overscan);
    const visible = Math.ceil(this._containerHeight / rowH) + overscan * 2;
    const end = Math.min(total, start + visible);

    return { start, end };
  }

  /** Handle scroll events on the virtual table body. */
  _onScroll = () => {
    if (this._renderRequested) return;
    this._renderRequested = true;
    requestAnimationFrame(() => {
      const body = this._root.querySelector('.virtual-table-body');
      if (!body) { this._renderRequested = false; return; }
      this._scrollTop = body.scrollTop;
      this._containerHeight = body.clientHeight;
      const range = this._calcVisibleRange();
      // Only re-render if visible range actually changed
      if (range.start !== this._visibleRange.start || range.end !== this._visibleRange.end) {
        this._visibleRange = range;
        this._renderVisibleRows(range);
      }
      this._renderRequested = false;
    });
  };

  /** Render only the rows in the given range into the <tbody>. */
  _renderVisibleRange(range) {
    const data = this._sortedData();
    const columns = this._columns;
    const colKeys = this._getColKeys(columns, data);
    const selectable = this.hasAttribute('selectable');
    const rowH = this._measureRowHeight();
    const totalHeight = data.length * rowH;
    const offsetY = range.start * rowH;

    const body = this._root.querySelector('.virtual-table-body');
    if (!body) return;

    // Update spacer to maintain scrollbar dimensions
    const spacer = body.querySelector('.virtual-spacer');
    ...
    for (let i = range.start; i < range.end; i++) {
      const row = data[i];
      rows.push(`
        <tr style="position:absolute;top:${i * rowH}px;height:${rowH}px;display:flex;align-items:center;left:0;right:0;">
          ${selectable ? `<td class="select-col" style="flex:0 0 2.5rem;display:flex;align-items:center;justify-content:center;height:100%;"><input type="checkbox" class="row-checkbox" data-idx="${i}" ${this._selectedRows.has(i) ? 'checked' : ''} aria-label="Select row ${i + 1}" /></td>` : ''}
          ${colKeys.map(k => {
            const key = typeof k === 'object' ? k.key : k;
            return `<td style="flex:1;padding:0 1rem;display:flex;align-items:center;height:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${row[key] ?? ''}</td>`;
          }).join('')}
        </tr>
      `);
    }
    ...
    this._bindRowCheckboxes();
  }

  /** Get column keys, auto-detecting from data if needed. */
  _getColKeys(columns, data) {
    return columns.length > 0 ? columns : (data.length > 0 ? Object.keys(data[0]) : []);
  }

  /** Bind change events on virtual row checkboxes. */
  _bindRowCheckboxes() {
  ...
  render() {
    ...
    const useVirtual = this._useVirtual;
    ...
    if (useVirtual && data.length > 0) {
      const rowH = this._measureRowHeight();
      const totalHeight = data.length * rowH;
      // Reset visible range on full re-render
      ...
      this._root.innerHTML = `
        <style>
          ...
          .virtual-table th.sortable { cursor: pointer; }
          .virtual-table th.sortable:hover { color: var(--i56-color-text); background: var(--i56-color-bg-tertiary); }
          .virtual-table th .sort-icon { margin-left: 0.25rem; opacity: 0.4; display: inline-block; width: 0.75rem; flex-shrink: 0; }
          .virtual-table th .sort-icon.active { opacity: 1; color: var(--i56-color-brand); }
          .virtual-table-body {
            overflow-y: auto;
            overflow-x: hidden;
            max-height: ${this.getAttribute('virtual-max-height') || '400px'};
            position: relative;
          ...
          .virtual-table-body tbody tr {
            border-bottom: 1px solid var(--i56-color-border);
            color: var(--i56-color-text);
            font-size: var(--i56-font-size-sm);
            box-sizing: border-box;
          }
          .virtual-table-body tbody tr:last-child { border-bottom: none; }
          :host([striped]) .virtual-table-body tbody tr:nth-child(even) {
            background: var(--i56-color-bg-secondary);
          }
          :host([hoverable]) .virtual-table-body tbody tr:hover {
            background: var(--i56-color-brand-light);
          }
          .select-col { flex: 0 0 2.5rem !important; justify-content: center; }
          input[type="checkbox"] {
        ...
        <div class="table-wrapper">
          <div class="virtual-table" role="grid">
            <thead>
              <div role="row" style="display:flex;">
                ${selectable ? `<div class="select-col" role="columnheader" style="display:flex;align-items:center;justify-content:center;padding:0.625rem 0;"><input type="checkbox" class="select-all-checkbox" aria-label="Select all rows" /></div>` : ''}
                ${colKeys.map(k => {
                  const key = typeof k === 'object' ? k.key : k;
                  const label = typeof k === 'object' ? k.label || key : key;
                  const isSorted = this._sortKey === key;
                  const icon = isSorted ? (this._sortDir === 'asc' ? '▲' : '▼') : '⇅';
                  return `<div class="${sortable ? 'sortable' : ''}" role="columnheader" data-col="${key}" style="flex:1;padding:0.625rem 1rem;display:flex;align-items:center;font-weight:600;color:var(--i56-color-text-secondary);font-size:var(--i56-font-size-xs);text-transform:uppercase;letter-spacing:0.05em;user-select:none;${sortable ? 'cursor:pointer' : ''}">
            ...
            <div class="virtual-table-body">
              ...
              <table style="width:100%;border-collapse:collapse;position:absolute;top:0;left:0;table-layout:fixed;">
                <tbody></tbody>
              </table>
            </div>
          </div>
      ...
      // Bind header clicks
      this._root.querySelectorAll('.virtual-table [data-col].sortable').forEach(th => {
      ...
      // Bind scroll on the virtual body
      const scrollBody = this._root.querySelector('.virtual-table-body');
      if (scrollBody) {
        scrollBody.addEventListener('scroll', this._onScroll, { passive: true });
      }

      // Re-measure row height after first paint
      requestAnimationFrame(() => {
        const firstRow = this._root.querySelector('.virtual-table-body tbody tr');
        if (firstRow) {
          const measured = firstRow.getBoundingClientRect().height;
          if (measured > 0 && measured !== this._rowHeight) {
            this._rowHeight = measured;
            this._scrollTop = this._root.querySelector('.virtual-table-body')?.scrollTop || 0;
            this._containerHeight = this._root.querySelector('.virtual-table-body')?.clientHeight || 0;
            const newRange = this._calcVisibleRange();
            if (newRange.start !== range.start || newRange.end !== range.end) {
              this._visibleRange = newRange;
              this._renderVisibleRows(newRange);
            }
          }
        }
      });
    ...
    // ---- Normal (non-virtual) rendering path ----
    this._root.innerHTML = `
      <style>
        :host { display: block; font-family: var(--i56-font-family); }
